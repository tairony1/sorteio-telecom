...
export default function CriarSorteioClient() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h2 className="text-3xl font-bold text-foreground mb-2">Criar Novo Sorteio</h2>
        <p className="text-muted-foreground">Preencha as informações em 5 etapas</p>
      </div>
        ...
        {steps.map((step, index) => (
          <div key={step.id} className="flex items-center flex-1">
            <div className="flex flex-col items-center flex-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold transition-colors ${currentStep >= step.id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                {step.id}
              </div>
              <span className={`text-xs mt-2 text-center ${currentStep >= step.id ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>
                {step.title}
              </span>
            </div>
            {index < steps.length - 1 && (
              <div className={`h-1 flex-1 mx-2 rounded transition-colors ${currentStep > step.id ? 'bg-primary' : 'bg-muted'}`} />
            )}
          </div>
        ))}
      </div>
          ...
          <Tabs value={`step${currentStep}`} className="w-full">
            <TabsContent value="step1" className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea id="descricao" placeholder="Descreva o sorteio..." rows={4} />
              </div>
              <div className="space-y-2">
            ...
            <TabsContent value="step2" className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="ticketsPorContrato">Tickets por Contrato</Label>
                <Input id="ticketsPorContrato" type="number" placeholder="5" defaultValue="5" />
              </div>
              <div className="space-y-2">
            ...
            <TabsContent value="step4" className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="limiteParticipantes">Limite de Participantes</Label>
                <Input id="limiteParticipantes" type="number" placeholder="500" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="criterios">Critérios de Participação</Label>
                <Textarea id="criterios" placeholder="Ex: Apenas clientes com contratos ativos" rows={4} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="restricoes">Restrições</Label>
                <Textarea id="restricoes" placeholder="Defina restrições ou regras especiais..." rows={4} />
              </div>
            </TabsContent>
            ...
            <TabsContent value="step5" className="space-y-4">
              <div className="bg-muted/50 p-6 rounded-lg space-y-4">
                ...
                <div className="space-y-2 text-sm">
                  <p><strong>Título:</strong> Sorteio de Natal 2024</p>
                  <p><strong>Data:</strong> 25/12/2024</p>
                  <p><strong>Total de Tickets:</strong> 1000</p>
                  <p><strong>Tickets por Contrato:</strong> 5</p>
                  <p><strong>Participantes:</strong> Até 500</p>
                  <p><strong>Prêmios:</strong> 3 prêmios cadastrados</p>
                </div>
              </div>
              <div className="bg-yellow-500/10 border border-yellow-500/20 p-4 rounded-lg">
                <p className="text-sm text-yellow-700 dark:text-yellow-400">⚠️ Após publicar, o sorteio ficará ativo e os participantes poderão resgatar tickets.</p>
              </div>
            </TabsContent>
          </Tabs>
