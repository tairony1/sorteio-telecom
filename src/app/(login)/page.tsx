        ...
        >
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/30 rounded-full px-6 py-2 mb-4">
              ...
              <span className="text-sm font-medium text-foreground">Acesso ao Sorteio</span>
            </div>

            <h1 className="text-4xl font-bold text-gradient mb-2">Bem-vindo!</h1>
            <p className="text-muted-foreground">Faça login para acessar seus bilhetes</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="space-y-2">
              <Label htmlFor="usuario" className="text-foreground">Usuário</Label>
              <Input id="usuario" type="text" placeholder="Digite seu usuário" value={usuario} onChange={(e) => setUsuario(e.target.value)} />
            </div>

            <div className="space-y-2">
              <Label htmlFor="senha" className="text-foreground">Senha</Label>
              <Input id="senha" type="password" placeholder="Digite sua senha" value={senha} onChange={(e) => setSenha(e.target.value)} />
            </div>

            <div className="flex items-center gap-2 py-3">
              <Checkbox id="lembrar" checked={lembrar} onCheckedChange={(v) => setLembrar(!!v)} />
              <Label htmlFor="lembrar" className="text-foreground cursor-pointer">Lembrar usuário</Label>
            </div>
